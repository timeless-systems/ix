  ...
  const fileIndexingStatuses: ConnectorIndexingStatus<FileConfig, {}>[] =
    connectorIndexingStatuses?.filter(
      (connectorIndexingStatus) =>
        connectorIndexingStatus.connector.source === "file_ng"
    ) ?? [];
            ...
            onSubmit={async (values, formikHelpers) => {
              const uploadCreateAndTriggerConnector = async () => {
                ...
                const response = await fetch(
                  "/api/manage/admin/connector/file_ng/upload",
                ...
                const [connectorErrorMsg, connector] =
                  await createConnector<FileConfig>({
                    name: "File_NGConnector-" + Date.now(),
                    source: "file_ng",
  ...
  return (
    <div className="mx-auto container">
      ...
      <div className="border-solid border-gray-600 border-b pb-2 mb-4 flex">
        <FileIcon size={32} />
        <h1 className="text-3xl font-bold pl-2">File_NG</h1>
      </div>
      <Main />
    </div>
  );
