...
_VALID_LANGCHAIN_FILE_EXTENSIONS = [".txt", ".zip", ".pdf"]
...
def check_file_ext_is_valid(ext: str, type) -> bool:
    if type == "langchain":
        return ext in _VALID_LANGCHAIN_FILE_EXTENSIONS
    else:
...
def write_temp_files(
    ...
    for file_name, file in files:
        ...
        if not check_file_ext_is_valid(extension, type):
            raise ValueError(
               f"Invalid file extension for file: '{file_name}'. Must be one of {(_VALID_FILE_EXTENSIONS if type == 'file' else _VALID_LANGCHAIN_FILE_EXTENSIONS)}"
            )
